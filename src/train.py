...
from tensorflow.python.tpu import tpu, tpu_feed, tpu_function, training_loop
...
def training_loop_repeat(n, body, inputs=None, infeed_queue=None, name=None, pass_index=False):
  ...
  def body_wrapper(i, *args):
    return [i + 1] + _convert_to_list(body(i, *args) if pass_index else body(*args))
...
def get_or_create_manual_step():
    with tf.variable_scope('', reuse=tf.AUTO_REUSE, use_resource=True):
        return tf.get_variable("manual_step", [], tf.int64, initializer=tf.zeros_initializer(),
                                             trainable=False,
...
def computation_func(params: ModelParameter, input_fn: typing.Callable,
  ...
  with tf.variable_scope('', use_resource=True):
    ...
    def _model_fn(i, global_step, manual_step, *args):
        ...
        if params.train:
            update_ops, learning_rate = get_optimizer(loss, params, global_step, manual_step)
        else:
            if params.use_language:
        ...
        if params.train:
            ...
            # step = tf.mod(current_step, tf.constant(params.grad_accumulation, dtype=tf.int64))
            ...
            with mtf.utils.outside_all_rewrites():
                ...
                return tf_loss, tf.group(comput_ops)
    ...
    def get_input_initializers():
      return [ds.initializer for ds in ds_iterator]

    def get_enqueue_ops_fn():
      assert len(datasets) == len(ds_iterator) == len(infeed_queues) == 0
      ...
      for sub_batch_i, host_id in enumerate(hosts_to_hold_ds):
          ...
          with ops.device(f"/job:worker/task:{host_id}/device:CPU:0"):
              ...
              dataset = input_fn(params, sub_batch_size, sub_batch_i,
                                      len(hosts_to_hold_ds))
      ...
      def enqueue_ops_fn():
          # For each sub-batch, create a SubBatchSlicer object.
          for sub_batch_i, host_id in enumerate(hosts_to_hold_ds):
              _ds_iterator = ds_iterator[sub_batch_i]
          ...
          infeed_queues.append(infeed_queue)
    ...
    enqueue_count = params.iterations * params.grad_accumulation
    enqueue_ops = wrap_computation_in_while_loop(
            get_enqueue_ops_fn(),
            n=enqueue_count,
            host_name=f"/job:worker/task:{hosts_to_hold_ds[0]}",
            parallel_iterations=1)

    input_initializers = get_input_initializers()
    ...
    def tpu_step(i, global_step, manual_step, loss, *values):
      del loss
      #values = infeed_queues[0].generate_dequeue_op(tpu_device=0)
      # unflattened_inputs = tf.nest.pack_sequence_as(feature_structure, values)
      loss, train_op = _model_fn(i, global_step, manual_step, *values)
      with tf.device(device_for_tpu_core()):
        with tf.control_dependencies([train_op]):
          loss = tf.identity(loss)

      step = tf.mod(manual_step, tf.constant(params.grad_accumulation, dtype=tf.int64))
      step = tf.equal(step, tf.constant(0, dtype=tf.int64))
      step = tf.cast(step, tf.int64)
      
      return global_step + step, manual_step + 1, loss, train_op
      

    @tpu_function.on_device_training_loop
    def tpu_loop():
      global_step, manual_step = tf.tpu.outside_compilation(lambda: (tf.train.get_or_create_global_step(), get_or_create_manual_step()))
      ...
      return training_loop_repeat(
          ...
          tpu_step,
          inputs=[global_step, manual_step, 1e7],
          infeed_queue=infeed_queues[0],
          pass_index=True)

    global_step = tf.train.get_or_create_global_step()
    manual_step = get_or_create_manual_step()

    compilation_state, computation = tpu.split_compile_and_replicate(tpu_loop,
                                                                     [[]] * params.num_cores,
                                                                     None, #infeed_queues[0],
                                                                     params.d_assignment,
                                                                     None,
                                                                     maximum_shapes=None)
    with tf.control_dependencies([
      global_step.assign_add(params.iterations, read_value=False)
      ]):
      train_op = tf.group(computation)

    ckpt_loader_hook = CheckpointLoaderHook(params.model_path)

    def log(msg):
      print(f'{params.tpu_name}: {msg}')

    tpu_shutdown_op = tf.tpu.shutdown_system()
    tpu_initialize_op = tf.tpu.initialize_system()
    ...
    if params.train:
        ...
        with tf.train.MonitoredTrainingSession(master=cluster_resolver.master(),
                                               hooks=[ckpt_loader_hook,
                                                      ExamplesPerSecondHook(every_n_steps=10, batch_size=params.train_batch_size * params.grad_accumulation),
                                                      tf.train.StepCounterHook(every_n_steps=10),
                                                      ] + hooks,
                                               config=session_config) as sess:

            class EnqueueForStep(threading.Thread):
              def __init__(self, session, step, blocking=True):
              ...
              def run(self):
                ...
                try:
                  ...
                  self.recover()
                  sess.run(enqueue_ops)
                self.log(f'Finished enqueueing batch')

              def recover(self):
                sess = self.session
                self.log('Enqueueing failed (did the TPU restart?), re-initializing TPU')
                sess.run(tpu_initialize_op)
                self.log('Re-running dataset initializers')
                sess.run(input_initializers)
                self.log(f'Retrying enqueue batch')
                sess.run(enqueue_ops)

              def log(self, msg):
                log(f'{msg} for step {self.step} thru {self.step+params.iterations}...')

            enqueue_cursor = [-1]

            infeed_threads = []

            def enqueue_batch(step, blocking=True):
              for thread in [thread for thread in infeed_threads if not thread.is_alive()]:
                infeed_threads.remove(thread)
              if enqueue_cursor[0] < step:
                enqueue_cursor[0] = step
                log(f'Prefetching enqueue for step {step}...')
                thread = EnqueueForStep(session=sess, step=step, blocking=blocking)
                infeed_threads.append(thread)
                return thread

            # log('Shutting down TPU...')
            # sess.run(tpu_shutdown_op)

            # log('Initializing TPU...')
            # sess.run(tpu_initialize_op)

            log('Compiling computation...')
            now = time.time()
            sess.run(compilation_state)
            elapsed = time.time() - now
            log(f'Compiled in {elapsed:.2f}s')

            log('Running input_initializers...')
            sess.run(input_initializers)

            log('Initializing summary...')
            summary.initialize(session=sess)

            for i in range(params.current_step, params.train_steps, params.iterations):
