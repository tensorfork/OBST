...
def get_optimizer(loss_list: mtf.Tensor, params: ModelParameter, manual_step
    ...
    shared_operations = loss_list[0].graph.operations

    for loss_idx in range(1, len(loss_list)):
        shared_operations = list(set(shared_operations) & set(loss_list[loss_idx].graph.operations))
                    ...
                    for inp, grad in itr:
                        ...
                        if params.use_PCGrad and len(loss_list) > 1:
                            if op in shared_operations:
                                flatt_grad_dim = mtf.Dimension("flatt_grad", size=grad.size)
                                grad_flat_shape = mtf.Shape([flatt_grad_dim])
                                grad_flat = mtf.reshape(grad, grad_flat_shape)
                                second_grad = variable(var, "second_grad", grad_flat.shape)

                                if (loss_idx + 1) < len(loss_list):
                                    update_ops.append(mtf.assign(second_grad, grad_flat))
                                    continue

                                else:
                                    inner_product = mtf.reduce_sum((grad_flat * second_grad),
                                                                   reduced_dim=flatt_grad_dim)
                                    proj_direction = inner_product / mtf.reduce_sum((second_grad * second_grad),
                                                                                    reduced_dim=flatt_grad_dim)
                                    grad_flat = grad_flat - mtf.minimum(proj_direction, 0.0) * second_grad

                                    grad = mtf.reshape(grad_flat, new_shape=var.shape)
