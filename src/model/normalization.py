...
from ..mtf_wrapper import einsum, reduce_mean, rsqrt, square, add, negative
...
def norm(args: BlockArgs) -> mtf.Tensor:
    ...
    scale = [rsqrt(add(reduce_mean(square(block_input), output_shape=normalized_shape), 1e-6)), block_input]
    if 'scale' in args:
